...
object MainClass {
  ...
  def Blocks(xs: Int*) = List(xs: _*)import scala.collection.immutable._
import scala.math.{abs, max, min}
import scala.concurrent.duration._
import util.control.Breaks._
...
object MainClass {
  ...
  def checkCollision(piece: pieceType, thisGrid: pieceType): Boolean = {
    if(piece.exists(block=> block(0) < 0)) {
      ...
      //piece.
      val intersect = thisGrid.intersect(piece)
  ...
  def movePiece(oldPiece:pieceType, newPiece: pieceType) = {
    var thisGrid: pieceType = grid.diff(oldPiece)
    if(!checkCollision(newPiece, thisGrid)) {
...
// ⬛
  ...
  def main(args: Array[String]): Unit = {
    ...
    while(!gameOver) {
      ...
      gameOver = checkCollision(currentPiece, grid)
        setInGrid(currentPiece)
      ...
      while(!touchBottom) {
        ...
        var newPiece = currentPiece
          ...
          movePiece(currentPiece, newPiece)
